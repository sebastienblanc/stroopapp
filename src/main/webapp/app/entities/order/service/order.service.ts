...
type RestOf<T extends IOrder | NewOrder> = Omit<T, 'startDate'> & {
...
export class OrderService {
  ...
  protected convertDateFromClient<T extends IOrder | NewOrder | PartialUpdateOrder>(order: T): RestOf<T> {
    return {
      ...order,
      startDate: order.startDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restOrder: RestOrder): IOrder {
    return {
      ...restOrder,
      startDate: restOrder.startDate ? dayjs(restOrder.startDate) : undefined,
    };
  }
